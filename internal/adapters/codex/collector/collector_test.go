...
import (
	...
	"github.com/GuanceCloud/agent-telemetry/internal/adapters/codex/buildinfo"
	"github.com/GuanceCloud/agent-telemetry/internal/adapters/codex/config"
	"github.com/GuanceCloud/agent-telemetry/internal/adapters/codex/model"
...
`), 0o644); err != nil {
	...
	body := []byte(joinJSONLines(
		...
		row("2026-06-03T10:00:02.000Z", "response_item", map[string]any{
			...
			"role": "assistant",
			"content": []any{
				map[string]any{"type": "output_text", "text": "我先读取 dashboard skill 的说明，然后再继续。"},
			},
		}),
		...
		row("2026-06-03T10:00:03.000Z", "response_item", map[string]any{
			...
			"role": "assistant",
			"content": []any{
				map[string]any{"type": "output_text", "text": "我已经读完 skill 说明。"},
			},
		}),
	...
	assertJSONEqual(t, root.Attributes[attrOutputMessages], []map[string]any{
		{
			"role":          "assistant",
			"parts":         []any{map[string]any{"type": "text", "content": "我已经读完 skill 说明。"}},
			"finish_reason": "stop",
		},
	})
