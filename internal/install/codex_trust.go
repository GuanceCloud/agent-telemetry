...
func trustCodexHookProcess(cmd *exec.Cmd, cwd string, timeout time.Duration) error {
	...
	if err := send(map[string]any{
		...
		"params": map[string]any{
			"clientInfo":   map[string]any{"name": "agent-telemetry", "version": buildinfo.Version},
	...
	for {
		...
		case message := <-messages:
			...
			case 2:
				...
				if len(entries) == 0 {
					return errors.New("Codex did not discover the agent-telemetry user hook")
				}
...
func isManagedCodexCommand(command string) bool {
	...
	return strings.Contains(normalized, "agent-telemetry") ||
